...
static void fmt_try_get_integer(char **fmt, int *num);
 ...
 * Return value: UNF_SUCCESS on success, UNF_ERROR otherwise
 */
 ...
 */
int unf_next_record_size(unf_file_t *file)
{
    if (file == NULL ||
    ...
    *n_bytes_read = 0;
    ...
    while (*p) {
        ...
        for (int i_repeat = 0; i_repeat < num_repeats; i_repeat++) {
            ...
            else { // data_ptr == NULL => skip bytes
                int err = fseek(file->file_ptr, n_bytes, SEEK_CUR);
                if (err != 0) {
                    return UNF_ERROR;
                }
            }
...
static int fmt_get_type_size(char **fmt, int *data_type, int *type_size, int *num_repeats)
{
    ...
    *num_repeats = 1;
    fmt_try_get_integer(&p, num_repeats);
    ...
    if (*p == 'c') {
        ...
        *data_type = TYPE_CHAR;
        ...
        fmt_try_get_integer(&p, type_size);
    }
    else {
...
// does not change argument if an integer number wasn't found
static void fmt_try_get_integer(char **fmt, int *num)
{
    char *p = *fmt;
